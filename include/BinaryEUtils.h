...
class BinE {
    ...
    public:
       ...
       void checkBinaryCovariates(BinE binE, CommandLine cmd, unordered_map<string, vector<vector<string>>> phenoMap, vector<string> sampleID, vector<long int> include_idx, int samSize, int phenoType, string phenoHeaderName, std::vector<string> covSelHeadersName, std::vector<string> covSelHeadersName_new, int Sq_new);
};
